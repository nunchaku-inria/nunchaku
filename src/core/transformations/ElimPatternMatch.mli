...
module Make(T : TermInner.S) : sig
  ...
  val tr_statement :
    env:env ->
    (term, term, <ty:[`Mono]; eqn:[`Nested]>) Statement.t ->
    env * (term, term, <ty:[`Mono]; eqn:[`Linear]>) Statement.t
  ...
  val tr_problem:
    (term, term, <ty:[`Mono]; eqn:[`Nested]>) Problem.t ->
    (term, term, <ty:[`Mono]; eqn:[`Linear]>) Problem.t
