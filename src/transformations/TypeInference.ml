...
exception IllFormed of string * string * loc (* what, msg, loc *)
...
let pp_stack out st =
  let pp_frame out t =
    fpf out "@[<hv 2>trying to infer type of@ `@[%a@]`@ at %a@]"
      A.pp_term t Loc.pp (Loc.get_loc t) in
  Utils.pp_list ~sep:" " pp_frame out st
...
module Convert(Term : TermTyped.S) = struct
  ...
  module TyEnv = struct
    ...
    let add_cstor ~env ~loc ~name c ty =
      if Hashtbl.mem env.cstors name then (
        ill_formedf ~loc ~kind:"constructor"
          "a constructor named %s is already defined" name;
      );
      Hashtbl.add env.cstors name (c,ty)

    let add_datatype ~env ~loc id cstors =
      if ID.Tbl.mem env.datatypes id then (
        ill_formedf ~loc ~kind:"datatype" "%a already defined" ID.pp id;
      );
  ...
  let rec convert_ty_ ~stack ~env (ty:A.ty) =
    ...
    match Loc.get ty with
      ...
      | A.Var (`Var v) ->
        begin match TyEnv.find_var ~loc ~env v with
          | Decl (id, t) ->
            ...
            U.ty_const ~loc id
          | Var v -> ill_formedf ~loc ~kind:"type" "term variable %a in type" Var.pp v
          | TyVar v ->
            unify_in_ctx_ ~stack
      ...
      | A.MetaVar v ->
        U.ty_meta_var ~loc (TyEnv.find_meta_var ~env v)
      | A.TyArrow (a,b) ->
        U.ty_arrow ~loc
    ...
    let rec eval ~subst ty =
      ...
      match TyPoly.repr ty with
        ...
        | TyI.Forall (v,t) ->
          ...
          let subst = add ~subst v (U.ty_var ~loc v') in
  ...
  let rec convert_term_ ~stack ~env t =
    ...
    match Loc.get t with
      ...
      | A.Fun ((v,ty_opt),body) ->
        (* fresh variable *)
        let ty_var = fresh_ty_var_ ~loc ~name:v in
      ...
      | A.Match (t,l,def) ->
        ...
        let m = List.fold_left
            (fun m (c,vars,rhs) ->
               ...
               let c, ty_c = TyEnv.find_cstor ~loc ~env c in
               ...
               let vars' = List.map
                   (fun v ->
                      ...
                      Var.make ~name ~ty:(fresh_ty_var_ ~loc ~name:"_"))
                   vars
               in
               let n_params_c = num_implicit_ ty_c in
               let ty_params = CCList.init n_params_c
                   (fun i->fresh_ty_var_ ~loc ~name:(Printf.sprintf "_param_%d" i))
               in
               let ty' =
        ...
        let def = match check_cases_exhaustive_ ~loc ~env ~ty:ty_t m def with
          | `Ok -> None
  ...
  and convert_arguments_following_ty ~stack ~env ~subst ty l =
    match TyPoly.repr ty, l with
      ...
      | TyI.Meta var, b :: l' ->
        ...
        (* convert [b] and [l'] *)
        let b = convert_term_ ~stack ~env b in
        let loc = Term.loc b in
        let ty_b = U.ty_exn b in
        (* type of the function *)
        let ty_ret = U.ty_meta_var ~loc (MetaVar.make ~name:"_") in
        unify_in_ctx_ ~stack ty (U.ty_arrow ~loc ty_b ty_ret);
        (* application *)
  ...
  let check_prenex_types_ ~loc t =
    if not (is_prenex_ t) then (
      ill_formedf ~loc
        "term `@[%a@]`@ contains non-prenex types" P.pp t
    )

  let check_ty_is_mono_ ~loc t =
    if not (is_mono_ t) then (
      ill_formedf ~loc
        "type `@[%a@]`@ is not monomorphic" P.pp t
    )

  let check_mono_var_ ~loc v =
    if not (is_mono_var_ v) then (
      ill_formedf ~loc "variable %a has a non-monomorphic type" Var.pp v
    )

  let generalize ~close t : _ * U.t Var.t list =
    ...
    let t, new_vars = List.fold_right
        (fun (_,var) (t,new_vars) ->
           ...
           MetaVar.bind ~var (U.ty_var ~loc var');
           ...
           let t = match close with
             | `Fun ->
               ...
               let ty_t = U.ty_exn t in
               U.fun_ ~loc ~ty:(U.ty_forall ~loc var' ty_t) var' t
             | `Forall -> U.forall ~loc var' t
             | `NoClose -> t
           in
           t, var' :: new_vars
        ) vars (t, [])
  ...
  *)
  ...
  let convert_spec_defs ~loc ~env (untyped_defined_l, ax_l) =
    ...
    let defined_l, env', spec_ty_vars = match untyped_defined_l with
      ...
      | (v,ty) :: tail ->
        ...
        let t_vars = List.map (U.ty_var ~loc) vars in
        let defined = Stmt.mk_defined ~attrs:[] id ty in
        (* locally, ensure that [v] refers to the defined term *)
        let t = U.app ~loc ~ty:(ty_apply ty t_vars) (U.const ~ty ~loc id) t_vars in
        ...
        let env', l = Utils.fold_map
            (fun env' (v',ty') ->
               ...
               let t' = U.app ~loc ~ty:(ty_apply ty' t_vars) (U.const ~loc id' ~ty:ty') t_vars in
    ...
    List.iter (check_mono_var_ ~loc) spec_ty_vars;
  ...
  let rec extract_fun_ t : _ list * _ = match Term.repr t with
  ...
  let rec extract_eqn ~f t = match Term.repr t with
    | TI.Bind (Binder.Forall, v, t') ->
      CCOpt.map
        ...
        (extract_eqn ~f t')
    | TI.Builtin (`Eq (l,r)) ->
      let loc = Term.loc r in
      begin match Term.repr l with
        | TI.Const f' when ID.equal f f' ->
          let vars, rhs = extract_fun_ r in
          Some (vars, List.map (fun v -> U.var ~loc v) vars, rhs)
        | TI.App (f', args) ->
          begin match Term.repr f' with
            | TI.Const f' when ID.equal f f' ->
              let vars, rhs = extract_fun_ r in
              Some (vars, args @ List.map (fun v ->U.var ~loc v) vars, rhs)
            | _ -> None
          end
        | _ -> None
      end
  ...
  let complete_args_ ~loc args ty =
    let rec aux i subst args ty = match args, Term.repr ty with
      | [], TI.Bind (Binder.TyForall, v, ty') ->
        ...
        let l, ty = aux (i+1) (Subst.add ~subst v (U.var ~loc v')) [] ty' in
        v' :: l, ty
  ...
  let prepare_defs ~loc ~env l =
    Utils.fold_map
      (fun env' (v,ty,l) ->
         (* convert the type *)
         let ty = convert_ty_exn ~env ty in
         let id = ID.make v in
         let v_as_t = U.const ~loc ~ty id in
  ...
  let convert_rec_defs ~loc ~env l =
    ...
    let env', l' = prepare_defs ~loc ~env l in
    ...
    let l' = List.map
        (fun (id,ty,ty_vars,l) ->
           ...
           let env'' = TyEnv.add_def ~env:env'' (ID.name id)
               ~as_:(
                 let ty_vars' = List.map (U.ty_var ~loc) ty_vars in
                 U.app ~loc (U.const ~loc ~ty id) ty_vars' ~ty:(ty_apply ty ty_vars'))
           in
  ...
  let ty_forall_l_ = List.fold_right (fun v t -> U.ty_forall ~loc:(Term.loc t) v t)
    ...
    let env, l = Utils.fold_map
        (fun env (name,vars,cstors) ->
           ...
           let ty = List.fold_right
               (fun _v t -> U.ty_arrow ~loc U.ty_type t)
               vars U.ty_type
           in
    ...
    Utils.fold_map
      (fun env (id,vars,ty_id,cstors) ->
         ...
         let ty_being_declared =
           U.app ~ty:U.ty_type ~loc
             (U.const ~loc ~ty:ty_id id)
             (List.map (fun v->U.ty_var ~loc v) vars')
         in
         ...
         let env, cstors = List.fold_left
             (fun (env,cstors) (name,ty_args) ->
                ...
                let ty' = ty_forall_l_ vars' (arrow_list ~loc ty_args ty_being_declared) in
                ...
                Utils.debugf ~section 3 "@[constructor %a: %a@]"
                  (fun k-> k ID.pp id' P.pp ty');
                TyEnv.add_cstor ~env ~name ~loc id' ty';
         ...
         in
         List.iter (check_mono_var_ ~loc) vars';
         (* remember the list of constructors for this type *)
         TyEnv.add_datatype ~loc ~env id cstors;
         check_ty_is_prenex_ ~loc ty_id;
         let tydef = {Stmt.
  ...
  let convert_copy ~loc ~env c =
    ...
    in
    let ty_id =
      arrow_list ~loc (List.map (fun _ -> U.ty_type) vars) U.ty_type in
    let ty_new =
      U.ty_app ~loc (U.const ~loc ~ty:ty_id id) (List.map (fun v->U.ty_var ~loc v) vars) in
    ...
    let ty_abstract = ty_forall_l_ vars (U.ty_arrow ~loc ty_of ty_new) in
    let concrete = ID.make_full ~needs_at:(vars<>[]) c.A.concrete in
    let ty_concrete = ty_forall_l_ vars (U.ty_arrow ~loc ty_new ty_of) in
    ...
    let wrt = match c.A.wrt with
      ...
      | A.Wrt_subset p ->
        ...
        (* [p : concrete -> prop] *)
        unify_in_ctx_ ~stack:[] (U.ty_exn p) (U.ty_arrow ~loc ty_of U.ty_prop);
        Stmt.Wrt_subset p
      | A.Wrt_quotient (tty, r) ->
        let r = convert_term_exn ~env r in
        (* [r : concrete -> concrete -> prop] *)
        unify_in_ctx_ ~stack:[] (U.ty_exn r) (U.ty_arrow_l ~loc [ty_of; ty_of] U.ty_prop);
  ...
  let convert_attr ~loc ~env a =
    let fail() = ill_formedf ~loc "ill-formed attribute %a" A.pp_attr a in
    match a with
      | ["max_card"; n] ->
      ...
      | ["infinite"] ->
        Stmt.Attr_infinite
      | ["approx_of"; id] ->
        begin match TyEnv.find_var ~loc ~env id with
          | Decl (id,_) -> Stmt.Attr_finite_approx id
          | _ -> ill_formedf ~loc "expected type identifier, got %s" id
        end
  ...
  let convert_statement_exn ~env st =
    ...
    let info = {Stmt.name; loc=Some loc; } in
    Utils.debugf ~section 2 "@[<hv2>infer types in@ `@[%a@]`@ at %a@]"
      (fun k-> k A.pp_statement st Loc.pp loc);
    let st', env = match st.A.stmt_value with
      ...
      | A.Decl (v, ty, attrs) ->
        ...
        check_ty_is_prenex_ ~loc ty;
        (* parse attributes *)
        let attrs = List.map (convert_attr ~loc ~env) attrs in
      ...
      | A.Def (a,b) ->
        ...
        let env, s = convert_rec_defs ~loc ~env
            [(a, A.wildcard ~loc (), [A.eq ~loc (A.var ~loc a) b])] in
