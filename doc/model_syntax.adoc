...
No hurry, but it would be a great feature. Other possible names for "?__" (in
the input language) would be "fail" or "abort" or "avoid" or "block".
...
Should the output be self-contained or should it repeat declarations from the
input, e.g. "data"? Repeating is what TSTP and CVC4 do. This can be
...
Nunchaku on the output) and connecting tools together. I have no strong
opinion here.
...
For bound variables, again I would suggest aggressive renaming. Perhaps the
same convention as for atoms can be used, but with (e.g.) "$" instead of "Q"
or whatever.
