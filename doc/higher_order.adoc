...
----
rec epsilon_nonempty : pi a. (a -> prop) -> prop :=
  epsilon_nonempty Q = epsilon_nonempty Q asserting
       Q (epsilon_nonempty Q)

rec iota_nonempty : pi a. (a -> prop) -> prop :=
  iota_nonempty Q = iota_nonempty Q asserting
       Q (iota_nonempty Q)
----

No congruence rules are needed for "iota_nonempty" when specializing; otherwise,
"epsilon_nonempty" and "iota_nonempty" are identical.
